...
CSBEConnection::CSBEConnection()
{
	...
	this->DECBThreadRunningEvent = CreateEvent(NULL,FALSE,FALSE,NULL);//DeviceCallback
...
/********************************************************************
Function: SetDeviceCallback	
--------------------------------------------------------------------
Description:	

  This function sets the device callback function.  The device
  callback function gets called each time a connected device gets 
  a new device.

********************************************************************/

void CSBEConnection::SetDeviceCallback(DIVICEEVENTCALLBACK pDeviceCallback)
{
	this->pDeviceCallback = pDeviceCallback;
}
...
CString CSBEConnection::RequestDeviceList(void)
{
	...
	{
		//WriteLogFile("Request Log File Error!");
...
void CSBEConnection::Disconnect(void)
{
	try
	{
		...
		{	
			...
			SetEvent(this->DECBThreadRunningEvent);
			Sleep(100);
...
bool CSBEConnection::Connect(CString address, unsigned __int16 port, CString ApplicationName)
{
	...
	try
	{
		...
		if(error == 0)
		{
			...
			CreateThread(NULL, 0, &DeviceCallBackThread, reinterpret_cast< void* >(this), 0, &ThreadID);
					...
					case FTDI_DATA:
						{
							...
							if(pConnection->pFTDICallback != NULL)
							{
								pConnection->RXDataList.AddTail(pPacket);
								SetEvent(pConnection->DCBThreadRunningEvent);
								SetEvent(pConnection->DECBThreadRunningEvent);
							}
...
DWORD WINAPI CSBEConnection::DataCallBackThread(LPVOID pPtr)
{
	...
	catch(...)
	{
		...
		AfxMessageBox("Exception caught in SBEConnection DataCallback Thread.",MB_OK|MB_ICONERROR);
	}
...
DWORD WINAPI CSBEConnection::DeviceCallBackThread(LPVOID pPtr)
{
	CSBEConnection *pConnection;
	try
	{
		...
		while(pConnection->ThreadRunning)
		{
			if(pConnection->pDeviceCallback != NULL)
			{
				//SetEvent(pConnection->DECBThreadRunningEvent);
				pConnection->pDeviceCallback(pConnection->RequestDeviceList());
			}
			WaitForSingleObject(pConnection->DECBThreadRunningEvent, 100);
		}
	}
	catch(CException* e)
	{
	...
	{
		//TRACE("***** Exception caught in SBEConnection DeviceCallback Thread *****\n");
		AfxMessageBox("Exception caught in SBEConnection DeviceCallback Thread",MB_OK|MB_ICONERROR);
	}
	return 0;
}
